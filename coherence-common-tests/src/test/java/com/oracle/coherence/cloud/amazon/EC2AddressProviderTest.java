...
import com.oracle.bedrock.junit.SystemPropertyIsolation;
...
{
    /**
     * Ensure system property changes are isolated between tests.
     */
    @Rule
    public SystemPropertyIsolation systemPropertyIsolation = new SystemPropertyIsolation();
    ...
    public void generateWKAListTest()
    {
        ...
        DescribeInstancesResult descResult       = Mockito.mock(DescribeInstancesResult.class);
        ...
        Mockito.stub(ec2.describeInstances()).toReturn(descResult);
        Mockito.stub(descResult.getReservations()).toReturn(reservationsList);

        ArrayList<Instance> instanceList = new ArrayList<Instance>();

        instanceList.add(mockInstance);
        Mockito.stub(mockReservation.getInstances()).toReturn(instanceList);

        ArrayList<Address> elasticIPList        = new ArrayList<Address>();
        Address            mockElasticIPAddress = Mockito.mock(Address.class);

        elasticIPList.add(mockElasticIPAddress);

        DescribeAddressesResult descAddressesResult = Mockito.mock(DescribeAddressesResult.class);

        Mockito.stub(ec2.describeAddresses()).toReturn(descAddressesResult);
        Mockito.stub(descAddressesResult.getAddresses()).toReturn(elasticIPList);

        Mockito.stub(mockElasticIPAddress.getInstanceId()).toReturn("wka-instance");
        Mockito.stub(mockInstance.getInstanceId()).toReturn("wka-instance");

        Mockito.stub(mockInstance.getPrivateIpAddress())
        .toReturn(LocalPlatform.get().getLoopbackAddress().getHostAddress());
        ...
        assertEquals(LocalPlatform.get().getLoopbackAddress().getHostAddress(),
                     wkaList.get(0).getAddress().getHostAddress());
    ...
    public void generateWKAListTestNoMatch()
    {
        ...
        Mockito.stub(mockElasticIPAddress.getInstanceId()).toReturn("elastic-instance");
        Mockito.stub(mockInstance.getInstanceId()).toReturn("another-instance");
    ...
    {
        System.setProperty("tangosol.coherence.ec2addressprovider.propertyfile", "ec2credentials.properties");
